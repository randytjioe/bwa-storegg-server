...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      ...
      if (!res_nominal) {
        return res.status(404).json({ message: "Nominal game tidak Temukan" });
      }
      const res_payment = await Payment.findOne({ _id: payment });
      if (!res_payment) {
        return res.status(404).json({ message: "Payment game tidak Temukan" });
      }
      const res_bank = await Bank.findOne({ _id: bank });
      if (!res_bank) {
        return res.status(404).json({ message: "Bank game tidak Temukan" });
      }
